...
import json
import io
from _yf_session import new_yf_session  # curl_cffi 浏览器指纹，绕 Yahoo 401 Invalid Crumb
...
def get_global_data(tickers, years=4):
    ...
    try:
        data = yf.download(tickers, start=start_date, end=end_date, progress=False, session=new_yf_session())['Close']
        if isinstance(data, pd.DataFrame):
            missing_tickers = [t for t in tickers if t not in data.columns or data[t].isnull().all()]
            if missing_tickers:
                import time as mod_time
                mod_time.sleep(1)
                retry_data = yf.download(missing_tickers, start=start_date, end=end_date, progress=False, session=new_yf_session())
...
def get_stock_metadata(tickers):
    ...
    def _fetch_one(t):
        try:
            stock = yf.Ticker(t, session=new_yf_session())
            fi = stock.fast_info
...
def get_arena_a_factors(tickers: tuple) -> dict:
    ...
    try:
        spy_hist = yf.Ticker("SPY", session=new_yf_session()).history(period="1y")
...
def get_arena_c_factors(tickers: tuple) -> dict:
    ...
    # SPY 基准：120日 + 250日收益
    try:
        spy_hist = yf.Ticker("SPY", session=new_yf_session()).history(period="2y")
        spy_prices = spy_hist["Close"].dropna().astype(float) if not spy_hist.empty else pd.Series(dtype=float)
...
def get_arena_d_factors(tickers: tuple) -> dict:
    ...
    # 先取 SPY 基准 20 日收益
    try:
        spy_hist = yf.Ticker("SPY", session=new_yf_session()).history(period="65d")
    ...
    def _fetch_one(t):
        try:
            hist = yf.Ticker(t, session=new_yf_session()).history(period="65d")
...
@st.cache_data(ttl=3600 * 4)
def fetch_macro_radar_timeseries(window: str = "1Y", profile: str = "waveform") -> dict:
    """从后端获取雷达指标历史时序（每日 RS / Z / 复合分），供 Page 0 §1.5/§1.6 使用。
    `window` ∈ {1M, 3M, 6M, 1Y, 5Y, 10Y}——每个窗口用不同 RS/Z 计算尺度
    （1M=RS_20d/Z_250d, 10Y=RS_252d/Z_750d）。
    `profile="dynasty"` 时所有跨度固定使用 RS_252d；后端按 (profile, window) 分桶缓存。
    排名由前端按选中组别动态计算。
...
def fetch_buyback_relay_timeseries(window: str = "5Y") -> dict:
    ...
    try:
        r = requests.get(
            f"{API_BASE_URL}/api/v1/macro/buyback_relay/timeseries",
...
@st.cache_data(ttl=3600 * 4)
def fetch_buyback_fcf_relay_timeseries(window: str = "5Y") -> dict:
    """回购 FCF margin 规则池接力图时序（king_score = 纯 Z(RS_210d)，年度 PIT 池内横截面排名）。
    池子 = 三判据 + FCF margin 前 40，年末重构次年生效。供「回购进攻」页调用。
...
@st.cache_data(ttl=3600 * 4)
def fetch_logr2_stable_pool() -> dict:
    """logR² 稳定页：年度 PIT 带鱼池 + 月末滚动 5Y logR2 面板。失败返回 {"success": False}。"""
    try:
        r = requests.get(f"{API_BASE_URL}/api/v1/macro/logr2_stable_pool", timeout=120)
        r.raise_for_status()
        return r.json()
    except Exception as e:
...
def fetch_tech_leader_relay_timeseries(window: str = "5Y") -> dict:
    ...
    try:
        r = requests.get(
            f"{API_BASE_URL}/api/v1/macro/tech_leader_relay/timeseries",
            params={"window": window},
            timeout=120,
        )
        r.raise_for_status()
...
def fetch_ndx100_pit_relay_timeseries(window: str = "5Y") -> dict:
    ...
    供「纳指100 接力」页调用。失败返回 {"success": False, "error": ...}。
    """
    try:
        r = requests.get(
            f"{API_BASE_URL}/api/v1/macro/ndx100_pit_relay/timeseries",
            params={"window": window},
            timeout=180,
        )
        r.raise_for_status()
...
def get_etf_rs20d(tickers: tuple) -> dict:
    ...
    import numpy as np
    try:
        spy_hist = yf.Ticker("SPY", session=new_yf_session()).history(period="30d")
        spy_prices = spy_hist["Close"].dropna().astype(float) if not spy_hist.empty else pd.Series(dtype=float)
    ...
    for t in tickers:
        try:
            hist = yf.Ticker(t, session=new_yf_session()).history(period="30d")
...
def gbdt_shap_detail(
    price_df,
    vol_df=None,
    meta_data: dict = None,
    pit_meta_records: dict = None,
    month_spec: dict = None,
    grade: str = "",
    z_seed_tickers: list = None,
) -> dict:
    """调后端 /api/v1/gbdt/shap_detail，取单月单档因子级 SHAP 矩阵。
    ...
    try:
        ...
        payload = {
            ...
            "meta_data":        _sanitize_floats(meta_data or {}),
            "pit_meta_records": _sanitize_floats(pit_meta_records or {}),
            "month_spec":       month_spec or {},
            "grade":            grade,
            "z_seed_tickers":   list(z_seed_tickers or []),
        }
        r = requests.post(
            f"{API_BASE_URL}/api/v1/gbdt/shap_detail",
