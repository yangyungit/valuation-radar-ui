...
工作流：
  ...
  3. 在 Streamlit 页面点「回填历史数据」重新生成后端 arena_history（universe.db）
  4. 再跑 pytest → 看是否改善 → 循环直到全部通过

数据源：默认从 Render API 取数（真相源），连不上才降级本地 data/arena_history.json。
设 ARENA_HISTORY_SOURCE=local 可强制只读本地；RADAR_API_URL 可指向本地后端。
...
def load_b_monthly() -> dict[str, list[str]]:
    """提取B组每月 Top N ticker 列表（优先 Render，降级本地）。"""
    history = load_arena_history(HISTORY_FILE)
    result = {}
...
class TestConvictionIntegrity:
    ...
    def test_recent_records_have_conviction(self):
        ...
        missing = []
        for mo in recent:
            # 仅 Top-N 持仓记录带 conviction（后端每月存 ~10 候选，4 名后无信念字段属正常）
            recs = history[mo].get(TARGET_CLS, [])[:TOP_N]
            for r in recs:
    ...
    def test_holders_match_latest_month(self):
        ...
        # 取最近一个有 B 数据的月：当前月 B 可能尚未算完（空），跳过避免误判
        b_months = [m for m in sorted(k for k in history if not k.startswith("_"))
                    if history[m].get(TARGET_CLS)]
        if not b_months:
            pytest.skip("无 B 组月度数据")
        latest = b_months[-1]
        latest_top = {r["ticker"] for r in history[latest].get(TARGET_CLS, [])[:TOP_N]}
        assert holders == latest_top, (
            f"_holders_B {holders} 与最新有 B 月 {latest} 的 Top 3 {latest_top} 不一致。"
        )
