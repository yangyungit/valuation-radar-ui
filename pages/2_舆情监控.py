...
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-bottom:20px">
  ...
  <div style="background:rgba(52,152,219,0.08);border:1px solid rgba(52,152,219,0.3);border-radius:10px;padding:16px">
    <div style="font-size:15px;font-weight:700;color:#3498DB;margin-bottom:10px">① 新闻采集层</div>
    <div style="font-size:13px;color:#ccc;line-height:1.7">
      每天从 <b>6 个数据源</b>并行拉取新闻原文：<br>
      · <b>GDELT GKG</b>：全球事件数据库，已预打行业标签<br>
      · <b>GDELT DOC API</b>：聚焦分析师估值词汇（EV/EBITDA、earnings beat…）<br>
      · <b>Google RSS</b>：实时财经新闻流<br>
      · <b>Finnhub / Alpaca / Polygon</b>：守护进程 7×24 小时持续采集<br><br>
      <span style="color:#888">目标：用多源互补覆盖盲区，避免单一数据源遗漏。</span>
    </div>
  </div>

  <div style="background:rgba(26,188,156,0.08);border:1px solid rgba(26,188,156,0.3);border-radius:10px;padding:16px">
    <div style="font-size:15px;font-weight:700;color:#1ABC9C;margin-bottom:10px">② 关键词匹配</div>
    <div style="font-size:13px;color:#ccc;line-height:1.7">
  ...
  <div style="background:rgba(243,156,18,0.08);border:1px solid rgba(243,156,18,0.3);border-radius:10px;padding:16px">
    <div style="font-size:15px;font-weight:700;color:#F39C12;margin-bottom:10px">③ TF-IDF 热词挖掘</div>
    <div style="font-size:13px;color:#ccc;line-height:1.7">
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
  ...
  <div style="background:rgba(155,89,182,0.08);border:1px solid rgba(155,89,182,0.3);border-radius:10px;padding:16px">
    <div style="font-size:15px;font-weight:700;color:#9B59B6;margin-bottom:10px">④ LLM 语义过滤（Gemini Flash 质检）</div>
    <div style="font-size:13px;color:#ccc;line-height:1.7">
      TF-IDF 找出的候选词会送入 <b>Gemini Flash</b> 做语义审判：<br><br>
      · 判断该词是否真的与<b>股票估值/市场事件</b>相关<br>
      · 过滤纯噪音词（"the"、"said"、人名、地名等）<br>
      · 输出"通过 / 拒绝"及理由<br><br>
      通过质检的词晋升为 <b>tfidf_auto</b> 级别词条，自动进入词典参与下一轮匹配。<br>
      <span style="color:#888">这是机器学习与规则系统的结合点：LLM 补上纯字符串匹配无法理解语义的短板。</span>
    </div>
  </div>

  <div style="background:rgba(231,76,60,0.08);border:1px solid rgba(231,76,60,0.3);border-radius:10px;padding:16px">
    ...
    <div style="font-size:13px;color:#ccc;line-height:1.7">
      ...
      · <b>价格动量</b>：RS 排名进入前列<br>
      · 两者同时满足 → 可结合叙事雷达（Tab 5）判断大象起舞候选<br><br>
      <span style="color:#888">最终输出的不是原始新闻，而是经过 5 层过滤的<b>结构化交易信号</b>。</span>
    </div>
  </div>
...
if active_phase == 4:
    ...
    if log_data:
        ...
        _row_px = 38
        _table_h = len(log_data) * _row_px + 46  # 46 = thead height
        table_html = (
...
if active_phase == 5:
    _phase_header(
        5, "叙事四象限雷达",
        "横轴 = 热度 (0-100)，纵轴 = 升温速度 (-100 ~ +100)，颜色 = 集中式 / 分布式",
    )

    # --- Shared quadrant label mappings (old Chinese → new Chinese) ---
    _Q_LABEL_MAP = {
        "风口正劲": "主升浪",
        "暗流涌动": "蓄势区",
        "见顶预警": "衰减区",
