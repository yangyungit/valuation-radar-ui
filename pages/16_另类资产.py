...
st.set_page_config(page_title="另类资产 PIT 接力", layout="wide")
...
st.title("₿ 另类资产池 PIT 接力图")
st.caption(
    "**池子**:另类资产池（加密/贵金属/大宗/矿股，含合成 BTCX），每月常驻全含,只在池内横截面排名。"
...
with st.sidebar:
    if st.button("🔄 强制刷新另类资产接力数据"):
        fetch_alt_assets_pit_relay_timeseries.clear()
        fetch_gbdt_oos_prices.clear()
        st.rerun()

_WINDOWS = ["3Y", "5Y", "10Y"]
window = st.radio("时间跨度", _WINDOWS, index=1, horizontal=True, key="alt_window")

with st.spinner("📊 加载另类资产池接力数据..."):
    ts = fetch_alt_assets_pit_relay_timeseries(window)

if not ts.get("success"):
    st.error(f"⚠️ 另类资产 PIT 接力数据暂不可用:{ts.get('error', '未知错误')}")
    st.stop()

# alt_membership 每月全含 → 直接当 pool_membership 用。
ts["pool_membership"] = ts.get("alt_membership", {}) or {}

_all_tickers = ts.get("tickers", {}) or {}
_tickers = _all_tickers
_dates = ts.get("dates", []) or []
if not _tickers or not _dates:
    st.warning("⚠️ 另类资产池母体数据为空")
    st.stop()
st.caption(f"资产池：加密/贵金属/大宗/矿股（含合成 BTCX），当前后端命中 {len(_tickers)} 只。")
...
# 排名/热力图用 10M 动量，月末收盘直接取后端面板 close_me（全历史）。
# 与「标普500+纳指100」页同口径 10M，保证拆分前后另类资产自身排名一致。
...
if _close_me.empty or len(_close_me) < 11:
    st.warning("⚠️ 后端 close_me 面板为空（需先跑 build_alt_assets_pit_relay_panel 并上传）")
    st.stop()
king_m = (_close_me / _close_me.shift(10) - 1.0)

# 趋势留任掩码：在任票月末价 > 自己 4 月均线才留（≈ MA80 日线）。
...
_label = "另类资产"
st.markdown(f"## 🏆 另类资产池（{len(_cols)} 只）")

render_group(_label, _cols, "alt_main",
