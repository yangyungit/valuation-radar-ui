        ...
        with tab:
            ...
            <div style='background:#111; border:1px solid #2a2a2a; border-radius:6px; padding:10px 18px; margin-top:6px;'>
                <div style='font-size:13px; color:#888; line-height:1.7;'>
                    ...
                    背景色带 = 四大剧本宏观裁决（基本面维度）；
                    折线颜色 = 技术形态状态机（价格维度）；
                    🟪 <b style='color:#8E44AD;'>图底紫色竖杠</b> = 变点检测「确认信号」（多变量 CUSUM 单日 n_t ≥ 4，对应「调阵型」时刻，详见 §2.6）。
...
注：早期版本要求强信号持续 5 个交易日确认，但 9 年历史从未触发；现已放宽为单日 n ≥ K 即确认（按规则 K=4 即四个变量同日 CUSUM 越界，仍属罕见事件）。
