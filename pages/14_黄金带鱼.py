...
st.error(
    ...
    "三条修补路都试过都不成立：① 改价格窗长度（短端 3/4/6 年样本外 Calmar 0.14/0.09/0.14，全低于 SPY 0.17；"
    "round14 把长端 7/8/10 年也扫了，同样没一格站得住）；"
    ...
    "**为什么偏偏近十年灵**（round13）：把定稿拆回四层逐层测，"
    "「四季全达标 / 排科技 / 按回撤取 2 只」这三个改动在近十年合计 +0.41 Calmar（0.64→1.05），"
    "在 2003-2015 合计 **−0.10**、全程也是 **−0.10**（0.21→0.11）——**没有一层在样本外是正的**。"
    "最原始那版（只看 12-31 单日、不排科技、不砍人的全池等权）全程 9.1 / −43.9 / **0.21**，"
    "反而几乎追平 SPY 的 0.23；一年多的调参把它调成了 6.5 / −60.6 / 0.11。"
    "**随机对照更直接**：每年从池里随机抓 2 只跑 500 次，定稿排到样本外 **33 百分位**"
    "（不如三分之二的抽签）、近十年 94 百分位、**全程 54 百分位（就是抽签）**。"
    "同样砍到 2 只但按字母序（纯抽签）在近十年只有 0.45、样本外却和定稿一样是 −0.03——"
    "**排序键的价值只在这十年存在，不是选股能力。**"
...
st.caption(
    ...
    "**但「那就拉更长」也不成立**（round14）：底座换回没调过参的原始版、起点对齐到 2006 重扫，"
    "7 年窗 2008 照样拿 CAT/OXY/PRU，当年 −46.4%，比 5 年窗的 −32.0% 和 SPY 的 −36.8% 都差；"
    "10 年窗全程 Calmar 0.30 看着赢 SPY 的 0.22，其实是样本外十年空池九年换来的"
    "（21 年空仓 10 年，CAGR 只有 7.6% vs SPY 11.1%）。"
    "全程 Calmar 随 3/4/5/6/7/8/10 年窗走 0.08/0.20/0.19/0.08/0.08/0.15/0.30 是锯齿不是高原。"
    "**加严门槛也换不来更精英的池**：2022 年 10 年窗照样吸进 30 只（5 年窗 32 只）、2009-2012 各窗口全归零——"
    "牛市末期膨胀、熊市底部归零是三根价格轴都滞后的必然结果，跟窗口长度无关。"
