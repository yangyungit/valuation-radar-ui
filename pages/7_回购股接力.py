...
st.caption(
    ...
    "**排名**:每月末在**组内**做横截面排名,**king_score = Z(RS_210d)** —— 纯动量(年化跑赢 SPY)。"
...
def _render_group(group_label: str, cols: list, kp: str):
    """对一个候选子池跑完整流程:组内横截面排名 → 热力图 → 奖牌榜 → 净值重建。
    kp = 该组所有 streamlit widget / plotly key 的前缀,避免两组撞 key。
    """
    cols = [c for c in cols if c in king_m.columns]
    if not cols:
    ...
    g_king = king_m[cols]
    g_rs = rs_m[cols]
    rank_m = g_king.rank(axis=1, ascending=False, method="min")
    ...
    fig.update_layout(
        ...
        title=dict(text=f"{window} 月末{group_label}接力 · 组内 king_score 排名 · {len(_ordered)} 只 · {len(_xlabels)} 月",
...
""", unsafe_allow_html=True)
    ...
    st.caption(
        "每月末按 king_score 选组内 Top2,顺延 1 月执行(去 look-ahead) · **进场门槛**:新进场必须当月在组内前 2(金/银) · "
    ...
    st.plotly_chart(
        hv.build_combined_fig(_nav_l, _nav_r, _navc, _spy_wk, f"{group_label} Top2 — 左右两列 50/50 合成 vs SPY"),
        use_container_width=True, key=f"{kp}_nav_combined",
    )
    st.plotly_chart(
        hv.build_stitched_fig(_seg_l, f"{group_label}接力 左列 (Slot 0)", _spy_wk, _price_cache, name_map),
        use_container_width=True, key=f"{kp}_nav_l",
    )
    st.plotly_chart(
        hv.build_stitched_fig(_seg_r, f"{group_label}接力 右列 (Slot 1)", _spy_wk, _price_cache, name_map),
