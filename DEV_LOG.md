...
**主表加「在发作的驱动」列**：按 `cat_to_key` 反查品类当前有没有活跃驱动打中，可可/咖啡/糖/棉花/谷物这几行现在显示「厄尔尼诺」，其余品类显示「—」。

**报警流水加「这类历史准不准」列**：格式「胜率（N 条，中位收益，闭眼买对照）」，样本 < 5 条显示「样本不足」，命中率 < 50% 前面加 ⚠️。期限结构类报警（翻转倒挂等三种）没有这一列的数据——已到期合约 yfinance 拉不到，回测不了。

**新增「现在有哪些驱动在发作」**：放在「清单自己的洞」里、共用瓶颈之前。显示当前 RONI（连同 ONI 一起标出，两者口径不同：JJA 2026 的 ONI 是 1.80、RONI 是 1.36）、档位、被打中的品类，并写明**厄尔尼诺不是做多信号**——主理人笔记实测 1992 年以来 10 次厄尔尼诺糖价 T+12 月中位 −4%，这里给的是波动窗口不是方向，且可可/咖啡/糖/谷物/棉花这 5 个不是 5 注是同一注下 5 遍。

**验证**：拿真实后端数据跑通全部 join 逻辑——6 个驱动品类正确映射到「厄尔尼诺」、黄金等不相关品类显示「—」；92 条近期报警全部能在胜率表里查到对照（价格分位类和代理类 0 条「样本不足」）；可可「进入紧张区」渲染成「⚠️ 39%（56 条，中位 -3.5%，闭眼买 +2.5%）」。

## 2026-09-11 供给紧度页补载体映射与历史事件，运费/铀/稀土不再是空白

后端见 `valuation-radar` 54936a7：品类 → 载体映射从 obsidian 笔记搬进 `rigid_list.py`，加了商品 / 环节 / 卡在哪 / 响应时间字段；`tightness_daily` 新增 `source` 列区分期货读数和代理读数；`backfill_events_raw.csv` 的 5 个历史事件进了 DB。`api_client.py` 加 `fetch_tightness_carriers` / `fetch_tightness_events` 两个 fetch。

**主表加「载体」列**：按 `tight_cats` 反查 `rigid_list.RIGID`，显示该品类前 3 个载体。代理品类（运费、铀、稀土等 27 个，没有活跃期货合约）的「近月溢价」列原来显示误导性的「—」，现在显示「无期货」；判读列走后端的固定措辞「代理读数——这是载体自己的涨幅和分位，不是现货紧度」，前端原样透传，没有二次改写。

**新增「这个品类能买什么」**：选中品类后展示商品 / 环节 / 卡在哪 / 响应时间 / 真紧度指标，再列全部载体的市场和类型（期货滚动 / 实物 / 期货合约 / 股票）。非美股载体前面加 🌐 标出来——这一期新开的口子，主理人需要一眼看出哪些要另外开户。「锡」和「镍与钴」现在能看到 `000960.SZ`/`TINS.JK`、`603993.SS`/`3993.HK`，笔记里「无美股载体只能看」「无 ETF」这两条结论作废。

**新增「历史上这个品类遇到过什么」**：每个事件标题固定格式「{事件名}（{日期}）— 卡的是「{stage_hit}」」，环节判断放标题不塞表格末列；标题下先给「这次全买等权 T+60 超额」的基线，再按 T+60 超额倒序列载体明细。红海事件标题是「卡的是运」，不是「集装箱运费」——环节和品类是两层。

**新增「清单自己的洞」**：页面最后一节，读后端 `grid_gaps()` 渲染三张表——商品缺哪些环节、根因写下来了但不是品类（如「晶粒取向硅钢」）、只能买股票或买不到的品类。漏一格会在这里显示成一行空白。

**验证**：`streamlit.testing.v1.AppTest` 接本地后端跑通，0 异常。定点核对：选中「VLCC 原油油轮运费」能看到 `BWET` 标「期货滚动」、代理读数固定措辞、俄乌事件标题「卡的是「采 + 运」」；选中「原油」主表载体列是 `CL=F · USO · XLE`，不是油轮股；选中「锡」「镍与钴」能看到 A 股/港股/印尼载体并标市场。

## 2026-09-11 主题簇每个气泡都写名字，新增「命名依据」

后端见 `valuation-radar` b2b281c：节点新增 `theme_title` / `theme_status` / `theme_review` / `member_link`。本页消费这四个字段，没动布局、受力解算和筛选参数。

**名字取值**：`cluster_name()` 优先 `theme_title`，老后端不返回该字段时退回 `theme_name`，再退回行业 + 代表票。图、悬停、逐月表、最新簇表统一走这个函数（最新簇表原来直接读 `theme_name`，判读结果进不去）。4012 个节点里 38 个显示判出来的正式名称，106 个显示「共同原因待确认」，3868 个沿用旧名。

**图上标签改成全画**：原来只给链末端写名字，互相压住的直接跳过——放大也不会恢复，等于把名字藏了。现在 `draw` 里每个节点一个标签，中间节点和只活一个月的都写，同名的月份也逐月重复。每 8 个原文字符折一行，分段各自 HTML 转义再拼 `<br>`（先转义再切会把 `&amp;` 从中间切断），13px，`bottom center`，`cliponaxis=False`，上下边距各加 40px。默认近三年 160 个标签、「全部」窗口滑块调到 1 是 4012 个，都无报错。**密集处会互相压住，靠滚轮缩放看，不再靠隐藏名字装作不挤。**

**新增「命名依据」**：按已选链的月份选节点，展示状态（有共同逻辑 / 混合待拆 / 共同原因待确认）、共同原因、不符合的成员及贴合度、来源日期与出处、拆分建议、备注。判过但没结论的显示待确认；没判过的提示「新闻库有整月空缺的月份判不了，不是分类有问题」；`theme_status=config_error` 显示「命名依据暂不可用」。

**链选择框放宽**：原来要求 `n_nodes >= 2` 且出生月份落在窗口内。判读过的 3991、4001 所在的链都只有一个节点，按旧条件根本选不到，依据也就查不了。改成「该链有任一节点在显示窗口内」即入选，空列表只提示不 `st.stop()`，不再挡住后面的最新簇表。改完选择框从 436 条可选。

**文案**：删掉「名字写在末端 / 放大后被省略文字会恢复」和写死的复核数量，改成按 `theme_status` 实时统计。「分叉」口径改成「重合 ≥ 0.50 优先续接，未续接但 ≥ 0.20 另建一条链并连上父节点，一个父节点连出多个孩子才叫成员分叉」。「死亡」一律改成「停止续接」（统计卡、链尾注释、死亡原因）——连续两月超额非正只是旧规则停止续接，不是主题死了。

**验证**：`streamlit.testing.v1.AppTest` 实跑四种情况全部无异常——正常数据、老后端剥掉四个新字段、所有节点 `config_error`、「全部」窗口 + 滑块 1。图上文字数 = 气泡数（160 = 160）。名称里塞 `<b>A&B</b>"注入"` 渲染成 `&lt;b&gt;A&amp;B&lt;/<br>b&gt;...`，实体没被折行切断。定点核对 4001（单节点链，状态有共同逻辑，4 条来源、2 个不符合成员、备注齐全）、3992 → 4000 两个月同名延续。

## 2026-09-10 主题簇页同步节点级名称，区分「没连出后继」和「样本到头了」

后端见 `valuation-radar` 407472e：`theme_names.json` 加了 `node_names`，分叉两枝和中途换业务的链按各自成员另起名称，不再共用根名。本页只改文案和已有字段的展示，没动布局、受力解算或筛选逻辑。

**× 的含义改正**：原来写「这一支到此为止，下个月再没有成员对得上的后继」，会被读成投资失败。实际当前图每个节点只画一个父节点，合流画不出来——AI 短枝的 56/73 只成员后来都进了下个月的大簇，只因为单父匹配没画出连线就显示成 ×。图注改成「未连出后继，可能是匹配时选了别的父节点，也可能未达到连接条件，不代表投资失败」，并去掉「近三年真正的分叉只有 12 处」这个写死的数字。

**空心圆 = 样本最后一个月**：2026-05 的节点本来就没有下月数据，跟真正断掉的 × 混在一起看不出区别，marker 换成 `circle-open`。近三年窗口里 circle 269 / × 259 / 空心圆 14。

**名称变化看得见了**：选择框中途改过名的链显示「首名 → 末名」，没改过只显示一次（近三年 95 条链里 26 条改过名）；逐月成员进出表在月份后加「主题」列。`cluster_name` 逻辑不变，只是 docstring 改成「优先用后端按节点解析的名称」。

**定点核对**（本机 8000 接口）：4000 · 2026-05 空心圆「半导体设备、算力硬件与电力」，3974 · 2026-02 ×「加密、航天、量子与矿业」，3991 · 2026-04 ×「加密矿企、云算力与航天量子」。成员数、超额、气泡大小、连线数量与改动前快照一致。

**「首名 → 末名」只看 `theme_name`**：方案原话是「首末名不同就都显示」，照做后发现走行业回退的短链里代表票每月都在换，选择框到处是「Steel 36%｜SHW/SNAP/PPG → Steel 36%｜TXN/SNAP/NUE」这种又长又没信息的标签。主理人定了只按后端起的 `theme_name` 判断，回退名只显示一次。近三年 95 条链里显示「→」的从 26 条降到 2 条：「半导体、加密与航天混合 → 半导体设备、算力硬件与电力」和「电子制造、光通信与设计软件 → 混合：电子制造、光通信与医药」。

## 2026-09-10 主题簇页接后端新字段：气泡上写「行业 占比｜代表票」

后端见 `valuation-radar` 同日条目（`MAX_MEMBERS` 60→150，新增 `industry_mix` / `sector_mix` / `label` / `biggest`）。

**名字规则**：最大行业占比 ≥25% 就写「行业名 占比｜成交额最大三只」，否则退回「涨得最猛三只」。分界线的理由是行业分类给不出横跨行业的叙事名——2023-06 的半导体簇标成 `Semiconductors 48%｜NVDA/AMD/AVGO` 是准的，但 2020-03 那个疫情居家簇（ZM/TDOC/ZS，超额 +16.5%）最大行业只占 16%，标「Electronic Gaming & Multimedia」纯属误导，这时候领跑票比行业名有用。占比一律写出来，让人自己判断名字可信度。

**门面票用 `biggest` 不用 `leaders`**：有个 67% 是半导体的簇，`leaders`（超额最高三只）是 PEN / GRMN / ERIC，写进名字看不懂；换 `biggest`（成交额最大三只）后是 TXN / ADI / NXPI。hover 里两个都显示。

**hover 加行业构成**：前 3 大行业的只数 + 相对母体的富集倍数。最新月份表格把裸「行业」列换成「行业构成」，并加「代表」列。

**近 3 年窗口规模变化**：节点 489 → 542，大气泡 354 → 410，背景小点 135 → 132，标签实画 16 → 21。右边距从 180 加到 240、x 轴右侧留白从 +0.5 加到 +1.5，否则末月那几个名字会被切掉。

## 2026-09-10 主题簇图改成可缩放拖动 + 气泡碰撞推开，× 只标链的末端

主理人反馈重叠盖太厉害看不清，想要 Obsidian 关系图谱那种能放缩拖动、节点互相挤开的状态，「时间轴能对上就行，上下距离可以晃动」。

**布局换成碰撞松弛**：原来同一格里的点按序号均匀铺开，现在按月份分列做 80 轮松弛——把点当圆盘，压到就沿连线方向推开，同时用 0.04 的回拉力往目标格拽。硬约束 x 不出所在月份 ±0.40、y 不出所属月龄 ±0.45，所以时间轴仍然对得上。近 3 年窗口同月互相压住的点对从 1264 降到 218，且没有一对盖掉对方一半以上（松弛耗时 0.16s，全部窗口 3570 个点 0.48s）。

**气泡尺寸改成显式像素**：不再用 plotly 的 `sizeref`/`sizemode="area"`（算出来的直径不可知，没法喂给碰撞检测），改成 `5 + (bubble_px-5) * sqrt(n/max_n)`，侧边栏加「最大气泡直径」滑块（默认 18px）。松弛用的 `px_x` / `px_y` 按图宽 760px、图高减上下边距估。

**开缩放拖动**：`dragmode="pan"` + `config={"scrollZoom": True}`。剩下的重叠靠放大解决——plotly 缩放时 marker 是固定像素，放大后点之间拉开、marker 不变大，重叠自然散开。

**× 改成只标链的末端**（`node_id not in has_child`）。原来按 `vine.died_month` 标，而 87% 的藤活一个月就被判死，结果满图都是 ×，近 3 年 354 个链上节点里几乎全打叉。改完只剩 131 个，就是真正断在那里的。

**名字加避让**：按链长从高到低放，估算文字包围盒（`len(text)*6.4+10` px 宽、13px 高），和已放的压上就跳过。近 3 年 21 个候选实际画 16 个，放大后被省略的会重新出现——这条写进图注了。

## 2026-09-10 主题簇页改画法：y 轴换成「长到第几个月」，默认近 3 年，顶端写领跑票

原来一条藤占一行、行按出生月排，结果整张图是斜着的台阶——因为 87% 的藤只活 1 个月（3050 条藤中 2646 条），最长 7 个月，28 年没有一条活满一年，所以每条藤都只有一两个点，行号又随出生月递增。主理人看到的「台阶」是数据形状，不是画错。

**改法**：y 轴不再是藤的槽位，改成节点沿 `parent_node_id` 链条从主轴长上来的高度（诞生月 = 第 1 月，往上一格 = 多活一个月）。分叉出来的新簇不回主轴，接着父节点继续往上长，「旧簇裂出新簇」在图上就是一条不断的斜线。

**关键发现**：按 `parent_node_id` 追（延续 + 分叉都算），最长一支能连 **29 个月**——2022-08 到 2024-12 的中概股，PDD/FUTU → IQ/BILI/TME → NTES/BABA → XPEV/NIO/LI → JD/BEKE，首尾成员还有 11 只重合。原来按「藤」看不到，是因为 `link_vines` 里 `died_month` 一旦标死就不许延续，而死亡规则是连续 2 个月超额中位 ≤ 0，一支主题回调两个月就被斩断、下个月的后继算新藤。**后端逻辑没动，只是前端换了追踪口径**。

**名字**：`vine.label` 用的 `top_industry` 是行业众数，但中位簇 12 只票横跨 8 个行业，只有 10% 的簇行业算集中，直接显示会编出「Insurance Brokers = NEM/FDS/CIEN」（金矿）、「Biotechnology = VMC/MLM/RACE」（建材 + 法拉利）这种。所以名字只写领跑三只票，行业前缀仅在 `n_industries <= max(2, n*0.3)` 时才加。

**侧边栏换了**：起始年份 + 存活门槛两个滑块，换成显示窗口（近 3 / 5 / 10 年 / 全部，默认近 3 年）+ 写名字门槛（默认第 4 个月）。数据统一按 `start=1998-01` 全量拉一次再前端切窗口——按窗口起点向后端要会丢掉窗口外的父节点，高度就算错了。

**近 3 年窗口实际规模**：489 个节点里 354 个在链上（大气泡），135 个是冒出来一个月就没后继的（灰色小点当背景），写名字的 21 个，最高一支 29 个月，图高 962px。

**为什么不去改后端让藤串长**：拿现有 JSON 试过几种串联规则，Jaccard 门槛从 0.50 降到 0.25、或换成覆盖率口径降到 0.30，中位寿命都还是 1 个月，能活 12 个月的只从 1 条涨到 5 条。churn 在聚类本身——母体 700 只每月只有约 164 只进合格簇，上月某簇成员下月只剩 38% 还在任一合格簇里，且被拆进 2 个不同簇，相邻月最佳 Jaccard 中位只有 0.125。要真串长得改 `build_theme_clusters.py` 的聚类（固定切 40 刀 + 成员数 8~60 过滤），这次没动。

## 2026-09-10 新增主题簇演化页（爬山虎图），接后端 theme_clusters 端点

新页 `pages/28_主题簇.py` 挂在侧边栏「大类观察」组末尾，`api_client.fetch_theme_clusters(start)` 走后端 `/api/v1/macro/theme_clusters`（4h 缓存）。后端见 `valuation-radar` 同日条目。

**顶部第一句就是免责，不许挪位置**：这页是观察工具不是买入信号，热簇领跑者后 63 日超额中位 −2.14%、胜率 45%（n=22031），比随便买一只流动股还差。

**四块**：统计卡（最新月份 / 当月活着的藤 / 新生 / 死亡）→ 爬山虎主图 → 单条藤的成员进出表 + 超额中位折线 → 最新月份合格簇列表。主图一条藤一条折线，y 轴槽位按出生月份排，点大小 = 成员数（`sizeref` 用后端 `params.max_members` 算死，换筛选不会跳变），点颜色 = 超额中位（`RdYlGn`，`cmin/cmax` 写死 ±30%，不用自动范围），灰细线 = 分叉，× = 藤在这里死了。

**侧边栏两个筛选只影响显示**：起始年份（默认 2016）、只看存活 ≥ N 个月的藤（默认 3）。不在前端重算簇也不重判死亡，业务逻辑全在后端。

**默认视图下的实际规模**（2016 起 + 存活 ≥ 3 个月）：1290 条藤里只有 30 条上图，12 条是从别的藤分叉出来的，30 条全部已死，最长的一条活 7 个月（2022-11 起的 GFI / FCX / WYNN）。**最新月份 11 个合格簇全是当月新生**，2026-04 的 8 个簇一个都没延续下来——主题簇本来就换得快，这不是 bug。
...
**起因**：主理人追问「SNPS 是 2024 年末选出来的、2025 年才崩，为什么不设计紧急替补」。后端 round6 实测中途换人三条路全部更差，但顺带查出真正的病根——六道门里的 `maxDD >= -40%` 在 5 年窗上算，2020 疫情那个坑要到 2025 年才滚出窗外，一批票被五年前的老坑一票否决，池子被压到 2 只。改成只看近 4 年后全程 Calmar 0.75 → 0.82。

**持仓变化：无**。2026 年生效池仍是 `GWW / PWR` 各 50%。变的是历史名单——2025 年从 GWW+SNPS 两只变成 AZO/GWW/ORLY/SNPS 四只，2022 年 16 → 17 只（多 LULU），其余年份一字不动。

**`p_dd` 列换了口径**：现在报的是近 4 年窗的最大回撤（跟门槛口径一致），不再是 5 年窗，所以表里同一只票的这个数字会比之前浅。标签已改成「价格近4Y maxDD%」避免误读。举例 AAPL 从 −27.3 变 −26.2。

**caption 换成 round6 的数字**：全程 19.0 / −23.4 / 0.82，3Y 28.1 / −12.5 / 2.25，5Y 17.3 / −23.4 / 0.74。新增两段：① 为什么回撤门只看 4 年（含 9 格稳健区的过拟合体检）；② 中途换人三条路的实测数字。五条警告里第 ② 条重写——**这次改善来自池子变大有人分摊，不是提前发现坏票，SNPS 在新口径下照样进 2025 名单**，别让页面读者误以为规则学会了躲雷。

**核验**：`launchctl kickstart -k gui/$UID/com.yangyun.vr-backend` 后 curl `/api/v1/macro/logr2_stable_pool`，返回 2025 = AZO/GWW/ORLY/SNPS、2026 = GWW/PWR、`golden_rule` 已含「近4Y maxDD>=-40%」、GWW `p_dd` = −22.6，与后端 JSON 一致。无新增 lint。

---

## 2026-09-08 page 14 黄金带鱼同步「四季全达标」：当前持仓从 3 只变 2 只，轴表加四季达标列

**范围**：`pages/14_黄金带鱼.py`（caption 全段重写、`_axis_row` 加「四季达标」列、`_missing_axes` 补四季分支、全部候选表改按 q_pass 排序）。后端见 valuation-radar `ecc239b`。

**起因**：主理人追问「名单为什么一年换一次、窗口为什么 5 年」，后端 round5 补测发现两个默认值从没做过对照，同时验证出「换名单仍一年一次、但要求四个季末全部达标」的新规则全程 Calmar 0.75 > 老规则 0.71。主理人当场拍板落地。

**持仓变化**：2026 年生效池 `GWW / PWR / WAB` 三只 → **`GWW / PWR` 各 50%**。WAB 12-31 那天六道门全过（p_r2 0.911、p_cagr 22.7、ni_cagr 25.11），但四季只达标 3 次，按新规则不进池。

**「缺哪条轴」列新增一种情况**：以前六道门全过就显示「全达标」，现在这类票可能因为别的季度掉链子而不进池，会显示「12-31 六道门全过，但四季只达标 3/4」。七轴数字仍是 12-31 那季的值，不改口径。

**caption 换成 round5 的数字**：全程 17.8 / −23.7 / 0.75（老规则 18.5 / −26.3 / 0.70），3Y 24.2 / −21.9 / 1.11，5Y 15.1 / −23.7 / 0.64。警告从四条加到五条，新增两条丑话：① 近 3Y 回撤 −21.9% 远差于老规则的 −9.2%，全砸在 2025 年 GWW+SNPS 两只票上；② 本次属年中换规则，回测覆盖的是每年年初建仓，年中切换不在回测范围内。
...
**范围**：`pages/21_价格台阶.py`（新文件）、`api_client.py` 新增 `fetch_close_series`。后端零改动。

**起因**：主理人问「点数图横向压缩时间，有没有反过来压缩涨跌幅度的画法」，想直观看出涨幅由哪几天贡献。做法是点数图的时间轴版：纵轴按固定涨跌幅划格，价格穿一格才走一步，横盘期间线完全平直，横向长度就是真实横盘天数（原版点数图/卡吉图没有时间轴，量不了这个）。

**定值**：格子大小做成滑块 0.5%~50%、默认 10%、步长 0.5（主理人原话要「x1 或 x0.5」，原脚本参数是倍数口径 ×1.5，×1 = 不变、×0.5 = 腰斩都不成立，故把参数改成「每格涨跌 %」）。回看年数 3/5/10/15/25 默认 10。换格用一格滞后判定（`p >= base*mult**(lv+1)` 才上跳），格内怎么抖都不动，不会在边界抖动。格子边界线仅在层级数 ≤ 30 时绘制，否则整张图糊。

**价格源**：走 yfinance 而非 Sharadar。查实：`POST /api/v1/gbdt/oos_prices` 传自选池 13 只样本只回 9 只，ETH-USD / BTC-USD / SMH / BRK-B 全缺，且末日 2026-07-01 缺最近 7 周——台阶图最新的跳格正落在这段。自选池 116 只含两个加密标的，Sharadar 根本没有。本页纯展示、不涉回测对齐、无退市票，符合 `architecture-constraints.mdc` 「纯实时活票展示才用 yfinance」。

**新函数**：`fetch_close_series(ticker, years)` 而非复用 `get_global_data`——后者 `data[data.index.dayofweek < 5]` 会滤掉周末，BTC/ETH 是 7×24 交易，滤掉就漏跳格（实测 ETH 十年有 916 个周末交易日）。

**核验**：mock streamlit 把页面完整跑一遍（`tmp_assets/test_page21.py`），四组参数 ETH-USD/10% · NVDA/10% · KO/2% · AAPL/50% 全过，断言价格始终落在 `[台阶, 台阶×mult)` 区间内（穿格必跳）。kaleido 导出真实渲染图核对视觉。踩到一个坑：竖线 x 里放 pandas Timestamp 过不了 plotly 的 JSON 序列化，改 `to_pydatetime()`。红绿竖线改为上跳走上半格、下跳走下半格，密集时才不会叠成灰色。无新增 lint。
...
**起因**：主理人在 带鱼.md 把 page 8 带鱼池手分四档，要求给黄金档（AAPL/LLY/TJX/COST/V/BRK.B/MA）做「动量排名 + 跌破自适应 MA 换仓」的轮动页。

**定值**：排名轴 = 12M 动量；留任通道 = 月末价 > MA6×(1−0.5×近12月波动)（回购稳定页同款常量）；**无进场门**——破线腾位当月立刻按排名补。回购页严进场门（MA4>MA15+下穿重置）在 7 只小池实测崩坏（10Y CAGR 3.1% / DD -43% / 空槽率 30%，小池腾位后长期填不满，这池子空仓就是亏）。月线回测（2016-07→2026-07，单边 200bps）：Top2 20.7 / -21.2 / 0.98，换仓 1.9 次/年；Top1 17.3 / -30.6 / 0.57 只留对照。

**警告**：池子是 2026-07 事后手挑的十年最漂亮票，等权 7 只拿住就是 22.8 / -14.7 / 1.56 的基线，**轮动版没跑赢它**——页面价值是持有纪律与换仓提示，不是证明轮动更赚。规则经 6 变体择优（严门/松门/无门/k0.25/12-1动量/榜内不限档），预期打折。

---

## 2026-07-22 page 8 FCF收益率稳定：加逐月 logR² ≥ 0.75 趋势门槛

**范围**：`pages/8_FCF收益率稳定.py`（成员 mask 加 `gate_mask = logr2_m >= LOGR2_GATE` + 两个 tab 的 caption）。后端零改动——`logr2_panel` 后端本来就在 `/api/v1/macro/logr2_stable_pool` 返回，前端此前只用 `fcfy_panel`，本次把 logr2_panel 也读进来当门槛。

**起因**：主理人看 PGR（长期不破月线 MA20，平滑）vs ADM（反复穿 MA20，震荡）问怎么只留强趋势票。查实：池按年重建有粘性，ADM 2023 年末以 logR² 0.83 进 2024 池，进池后趋势掉头（2024 全年 logR² 0.83→0.28）却整年留在候选，Top1 在 2024.5–2025.1 一直持有它。逐月 logR² 门槛当月就把它踢出候选。

**定值**：`LOGR2_GATE = 0.75`（趋势顺滑度门槛）。平台扫描 [0.65,0.80] 是平台、0.85+ 急塌（Top1 到 0.85 DD -18→-28.8），取平台中心。基线自验：无门槛 Top2 17.1/-21.2/0.81、Top1 18.1/-18.3/0.99 逐位复现 caption。加门槛后 Top2 17.4/-21.2/0.82（换手 0.87→0.98）、Top1 22.8/-18.3/1.25（换手 0.98→0.65）。

**警告**：增益主要来自躲开 ADM 2024 单次，DD 全程未改善（仍 -21.2%/-18.3%）；真实价值是持仓更集中在平滑趋势票 + 降换手，非提升收益。幅度别外推。

---

## 2026-07-21 page 17 风险预警：加防守腿池宽度警报（round17 落地，人工信号不接引擎）

**范围**：`pages/17_风险预警.py`（新增第 4 条带 + 宽度折线图 + 说明表一行）。后端零改动（复用 `fetch_logr2_stable_pool` 池子 + `get_global_data` 价格，纯前端计算）。

**起因**：round17（valuation-radar，commit 9a96397）证伪防守股个股级止损（54 段 whipsaw、C/D 变体全输），池宽度门只在 2008 式慢熊有真实边际（空仓段 -0.5% vs 裸奔 -57.7%）但 V 形样本年亏 2.6pp 保费——判读为不接自动降仓，只做人工警报。

**口径**：page8 带鱼池非科技成员，月末收盘 > MA6×(1−0.25·σ12) 的占比；宽度 <40% 报警、回 ≥50% 解除（滞回），有效成员 <10 不切换。自验：前端历史报警段（2018-12 / 2020-03~04 / 2022-01~06 / 2022-09~10）与回测 risk-off 段同月对齐，当前 2026-07 宽度 83% 安全。caption 写明：慢熊有反应时间（2008-07 触发离底 8 个月）、快崩别照着砍（2020-03 触发时底已过）。
...
**做法**：仓位 = min(1, 目标年化波动 ÷ 策略净值近 20 日已实现波动)，无杠杆，缩掉部分持现金（年化 4%，同熊市防御口径）。波动率 shift 1 天防未来函数。目标波动做成滑杆（5%-80%），默认值 = 该策略自身波动中位数（数据驱动，不硬编码）。页面输出「当前建议仓位」+ 回撤/收益对照 caption。叠加顺序：熊市防御（GBDT 清仓/旧闸门减半）在前、波动率目标在后——闸门是预测式全有全无，这层是反应式连续降仓，互补兜底。

**动机**：动量策略的深回撤集中在高波动段（动量崩溃），波动率目标是文献验证过的对症手段（Barroso & Santa-Clara 2015）；GBDT 闸门 miss 时（P 不过 0.5 一点不减）此层仍然生效。

**验证**：合成数据自验（前 300 天 15% 波动 / 后 300 天 60% 波动）：低波段平均仓位 1.0、高波段 0.34，最大回撤 -83.2% → -44.8%，scale 无 NaN 无泄漏。`ReadLints` 无新增错误。熊市防御对照 caption 同步改为与波动率目标分层展示（满仓→防御→波动目标三级对照）。

## 2026-07-13 | 回购稳定页进场门加个股波动缩放：MA4 > MA15×(1+0.125×近12月波动)

**范围**：`pages/8_回购稳定.py`（进场门公式 + 常量 `_ENTRY_VOL_K=0.125` + caption/注释）、`buyback_relay_core.py`（`render_group` 新参 `entry_vol_scale`/`entry_vol_desc`：caption 文案 + MA 长窗口扫描重建门时乘同一缩放，保持扫描口径与实际一致）。

**起因**：主理人指出出场已按个股波动自适应，进场却是一刀切 MA4>MA15。回测（`tmp_entry_compare.py`，10Y 2016-07~2026-07）：直接把出场通道搬来当进场（收回下沿/MA6/上沿即可买）全样本更差（+301%→+225~233%，Calmar 0.92→0.5~0.67）——崩盘后均线跟着塌、门常开，缺趋势确认；保留 MA4/MA15 骨架、门槛按波动缩放 MA4 > MA15×(1+c×vol) 则有提升，系数扫 [-0.75,1.0] maximin 最优 c=0.125：总收益 301%→339%、回撤 16.0%→15.0%、3/5/10Y Calmar 0.80/0.85/0.90→1.10/1.08/1.03。

**保留意见**：与一刀切版持仓只差 10/122 个执行月，超额集中在 2023 年（更严的门把 MO 挡在场外、仓位让给 LOW/BKNG），样本薄；平台 0.125~0.25 不宽，c≥0.5 只有 3Y 好看（过拟合症状）。

**验证**：`ReadLints` 无新增错误；页面公式与回测脚本 c=0.125 变体逐项同式。其他调用方（pages 7/8_fcf/9/10/14/16）不传新参，零影响。
...
- `api_client.py.fetch_dynasty_double_dragon`：加 `n_holdings: int = 2` 参数，并入 `params`（`@st.cache_data` 自动纳入缓存键）。
- `pages/0_宏观雷达.py`：控制区加「持仓数量」滑杆（2~5，默认2），下附诚实小字；K 滑杆 `min_value` 动态跟随 N；`fetch_dynasty_double_dragon` 加 `n_holdings=_dd_n`；顶部 caption 随 N 显示「最强 N 只」；当前持仓卡、净值图（`topn`）、统计卡对照（`front_map.get(N)`）、持仓时间带全部泛化为 N 槽。

---

## 2026-06-11 | C组双龙持仓可信度与展示诚信修复

**范围**：不改变双龙策略、K、排名信号、成本或 API 请求参数，只同步后端前视/统计修复后的用户可见口径。

**改动**：

- Page0 开关「风险保护」改名为「个股趋势过滤」，帮助文案明确“不合格股先由其他合格股补位，合格股不足 2 只时缺少槽位才持有 BIL”；BIL 卡改为「无足够合格股票」，删除市场级“避险”暗示。
- 持仓标题改为「截至 YYYY-MM 信号的模拟持仓」；展示共同价格截止日，`is_stale=true` 时醒目提示过期。10Y 数据不足时保留窗口名称，仅显示「请求10Y｜实际约8.3Y」小字。
- `C组双龙持仓_执行plan.md` 同步修正 BIL 补位顺序、年均换手公式、有效 BIL 测试和人工验收口径。`api_client.py` 请求签名及缓存键不变。

后端修复后默认参数（12M/K30/过滤开/不再平衡/10bps）年化为 3Y 65.3%、5Y 43.2%、10Y实际8.31Y 30.4%；入指数前持仓槽月均为 0。旧版页面净值、年化、换手和 frontier 作废。
...
- `pages/0_宏观雷达.py` §1.6 caption 两处文案：把「对 25 个板块做横截面排名」改为「在 25 个板块内做横截面标准化与排名」；king_score 公式说明补「两项都只在这 25 个板块内做 Z-Score（不掺美债/BTC 等跨资产，否则小众盘容量惩罚被摊薄），RS 用真实美股交易日 252 日动量」。
- 纯文案改动，无逻辑变化；king_score 数值由后端新口径提供。

**契约**：依赖后端新口径（后端先部署）。历史金牌序列因口径修正会变（最近 13 月 URA 7 金→4 金，SMH 升至 7 金），属预期内的「变正确」，已全量核验。

---

## 2026-06-10 | Page0 王朝接力图新增 1Y / 2Y / 3Y 时间跨度

**动因**：§1.6 王朝接力图原先只有 5Y/10Y。新增近 1-3 年观察窗口，同时保持已审计的 RS_252d 王朝口径，不影响 §1.5 波形图短窗口的快速 RS 配置。

**改动**：

- `api_client.fetch_macro_radar_timeseries(window, profile="waveform")` 新增 `profile` 参数并透传后端。
- Page0 保留 `_radar_ts_by_window` 给 §1.5 波形图，新增 `_dynasty_ts_by_window`，按 `profile="dynasty"` 预取 1Y/2Y/3Y/5Y/10Y。
- §1.6 王朝接力图和王朝龙头股两个 tab 统一读取 dynasty 数据集；radio 增加五档，默认仍为 5Y。

**已知偏差**：1Y 只有约 12 个月末快照，连续戴金满 3 个月的王朝样本较少，多为接力段或脉冲；短跨度下进行中区间的快照时点差更显眼；候选池仍是当前成分快照回看历史，幸存者偏差与原 5Y/10Y 同源。
...
1. 上一版双 RS 加冕门槛(RS_252d > 0 AND RS_63d > 0)实测过严,主理人截图显示几乎全军灰块,「当前无王」。**回滚到单 RS_252d > 0 门槛**
2. 主理人质疑「Z 高时减分降级」与「Z 高总分更高」逻辑矛盾。**主理人对**——查后端 `macro_engine.py` line 1069 确认:
   - `z = (price - MA_z) / STD_z` = **价格 Z-Score**(价格相对历史均线偏离),不是估值 Z-Score
   - Z 高 = 价格远高于均线 = **强势加分**(趋势加速突破),不是「估值贵被减分」
   - `composite = RS + Z` = **动量 + 趋势加速** 双重追涨,**Z 跟 RS 同向**,不存在刹车
3. URA 横盘半年仍戴金的真正原因订正:
   - 不是「指标延迟,别人 RS 没及时被发现」
   - 是「universe 里其他板块 1 年累计涨幅都没追上 URA」——横截面排名是数学事实,每月重新算,无延迟
   - 这是动量指标的固有特性,**动量图天生追涨,无法识别登顶横盘**

**改动**:

1. `pages/0_宏观雷达.py` §1.6:
   - **加冕门槛回滚**:`_gold_mask = (_rank_m == 1) & (_rs_m > 0)`,删除 `_gold_63` 那段。`_demoted_mask` 同步改回 `_rs_m <= 0`
   - **保留 RS_63d 计算和 hover 显示**(标注「仅参考,不进门槛」)——hover 字段对主理人观察"短期 RS 是否还在跑赢"仍有信息价值
   - **hover「降级原因」简化**:只有 RS_252d ≤ 0 的 Top1 才显示「降级(熊市无王)」,RS_63d ≤ 0 不再触发
   - **主 caption 订正**:
     - 删除「双 RS」「为什么双 RS」段落
     - 添加「Z = 价格相对历史均线偏离,**与 RS 同向加分**」——纠正之前误导描述
     - 添加「**已知局限**:RS_252d 1 年回望,登顶横盘的板块仍会戴金」——透明说明动量指标缺陷
   - **🅱️ 纯 RS caption 全面订正**(原描述全错):
     - 删除「Z-Score 在压制」「已涨贵被估值减分」等错误描述
     - 新增「Z 是价格相对均线偏离,跟 RS 同向加分,不刹车」
     - 🅰️ 抢先戴金的真正语义:RS 非 Top1 但 Z 极大(价格突破均线)把它推上 Top1
     - 推荐主理人找时代之王用 🅱️:语义最直观(就是「过去 1 年涨得最多」)
   - **摘要读法订正**:删除 RS_63d 描述,加注「RS_252d 1 年回望特性 + 后续可能用 MA60 趋势信号补刹车」

**遗留问题**:

- URA 横盘戴金的核心问题仍未解决,只是回滚到「至少有金牌可看」的状态
- **未来方案**:可考虑加 ETF 价格 vs MA60 (10 周均线)趋势刹车:`ETF 月末价 < MA60` 时降为灰。这比 RS_63d 容差大,正常回调不会跌破 MA60,但登顶横盘半年会跌破
- 或:RS_63d > -3% 之类的宽松容差版本,但需要回测验证不误杀

**教训(给自己)**:在涉及具体公式方向时,**先 grep 后端源码确认**,不要凭直觉解释方向。"Z-Score 估值刹车"是金融通用语,但本项目的 Z 是价格 Z,不是估值 Z,语义完全不同。

---

## 2026-05-28 | §1.6 王朝接力加冕门槛升级:双 RS 过滤(登顶横盘自动降级)

**动因**:主理人发现 URA 上涨到顶部后横盘半年仍连续戴金。根因是 RS_252d 是 1 年回望窗口,只要 universe 里 1 年涨幅没人超过 URA,即使 URA 已经横盘 RS_252d 仍是 Top1。**RS 的固有滞后特性,需要短期 RS 过滤刹车**。

**改动**:

1. `pages/0_宏观雷达.py` §1.6 tab1:
   - **新增 RS_63d 前端自算**:用顶层 `df_prices`(10 年价格),公式与 macro_engine 一致 `RS_63d = ETF.pct_change(64)*100 − SPY.pct_change(64)*100`,reindex 到 `_dyn_idx`。无需改后端
   - **加冕门槛升级**:`Top1 AND RS_252d > 0 AND RS_63d > 0` → 戴金,否则降为灰。两个对照版本(🅰️ RS+Z、🅱️ 纯 RS)同步生效
   - **hover 增强**:Top1 但降为灰的格子额外显示「降级原因」(RS_252d ≤ 0 / RS_63d ≤ 0 / 两者皆负),让主理人能区分「熊市无王」vs「登顶横盘」
   - hover 多显示一行 RS_63d 数值
   - caption / 摘要读法同步更新「双 RS 加冕门槛」描述

**为什么不直接用 Z-Score 刹车**:Z-Score 在 🅰️ 版本已经在工作,但从主理人对比截图看 URA 还是大量戴金,说明 Z-Score 对「横盘高位」的检测不够及时。RS_63d 是直接、短期、机械的过滤,跟 Z-Score 互补不冲突。

**后端**:无改动

**验收**:

- URA 2024-2025 横盘期金块大幅减少(RS_63d 在 2024 后半年开始转零或转负)
- 🅰️ RS+Z 版本变化最大:Z-Score 已经在压制了,加上 RS_63d 双保险后金块更少
- 🅱️ 纯 RS 版本变化也明显:登顶横盘的板块从大块金变成金/灰交替
- hover 灰色 Top1 格子显示「Top1 但降级:RS_63d ≤ 0」之类的提示
- 两个版本「当前在位之王」描述变成「Top1 + RS_252d > 0 + RS_63d > 0」

**待观察**:

- 双 RS 过滤后,真王朝(如 2020-2022 半导体)的连续金块是否仍然完整(应该不受影响,因为期间双 RS 都为正)
- 是否过滤过猛,把一些主升浪中的正常回调误杀。如果是,可考虑放宽到 `RS_63d > -2%` 或类似容差

---

## 2026-05-27 | Page 0 §1.5 撤掉采样粒度联动 + §1.6 加纯 RS 对照版本

**动因**:

1. §1.5 试了「1Y=周末/5Y/10Y=月末」联动后,主理人觉得「视觉很糙」(月线下波形抬升/下沉的细节丢失,ZigZag 主升浪段在稀疏点上看不出连贯性)。**回滚为全日线**
2. §1.6 王朝接力图主理人想看「按纯 RS 排名」vs「按 RS+Z 合成排名」的对比,看 Z-Score 在哪些时点替自己抢跑或刹车

**改动**:

1. `pages/0_宏观雷达.py`:
   - **撤掉 §1.5 resample**:删除 `_RESAMPLE_RULE = {"1Y": "W-FRI", "5Y": "ME", "10Y": "ME"}` 那块逻辑,所有 tab 均日线渲染。caption 同步删除「采样粒度自动联动」一行
   - **§1.6 tab1 重构为函数**:把渲染逻辑包成 `_render_relay(metric_df, metric_label, hover_metric_label, key_suffix)`,接受任意每日时序作为排名 metric。共用变量(`_d_rs / _d_name_map / _picked_d / _dyn_idx`)外提,函数内自取月末快照
   - **§1.6 tab1 调两次函数**:🅰️ 按 RS+Z 合成排名(主图,与 §1.5 一致)/ 🅱️ 按纯 RS_252d 排名(对照,无 Z-Score 估值偏离)
   - **加冕门槛**两个版本都用 `RS_252d > 0`(保持一致,熊市都无王)
   - `streamlit.plotly_chart` 加 `key=f"fig_dyn_{key_suffix}"` 避免重复 key 冲突;`st.expander` label 含 metric_label 自然区分
   - §1.6 主 caption 加一行说明「下方两个对照版本」

2. **§1.6 tab2(王朝龙头股)未动**:它依赖 dynasty_leaders 后端 API,仍基于 RS+Z 合成排名识别王朝期。如未来需要按纯 RS 钻取龙头股,可加 metric 参数到 API 调用

**后端**:无改动

**验收**:

- §1.5 全部 tab 回到日线密集渲染,主升浪着色完整
- §1.6 tab1 上半:🅰️ RS+Z 接力图 + 摘要(与原版相同)
- §1.6 tab1 下半:🅱️ 纯 RS 接力图 + 摘要,Y 轴排序通常会跟上半不同(累计戴金月数不同)
- 两张接力图的金块位置差异 = Z-Score 的判断点。差异越多说明 Z-Score 越活跃
- §1.6 tab2「王朝龙头股」不变,仍按 RS+Z 识别王朝期

**怎么用对照图**:

- 金块**位置一致** → Z-Score 没在抢跑或刹车,RS 单维度足够定王
- 🅰️ **抢先戴金** → Z-Score 提示估值低位的板块被加分(便宜+强,提前称王)
- 🅱️ **抢先戴金** → Z-Score 在压制(已涨贵的板块被估值减分,延迟称王或降为银)
- 差异点 = Z-Score 在替你做的判断,主理人回溯看是否抢对/刹错,可决定是否调整 RS:Z 权重

---

## 2026-05-27 | Page 0 §1.5 强势波形熵减 + 新增 §1.6 王朝接力图

**动因**：主理人想从「广角扫场」转向「找时代之王」。25 条线全开 + 日线密集，看不出谁是真王朝。三件事一起做：

1. **§1.5 默认高亮砍半**：彩色线从 `Top8 + Rising5 = 13 条` 降到 `Top5 + Rising2 = 7 条`，更聚焦
2. **§1.5 采样粒度按 tab 联动**：1M/3M/6M=日线、1Y=周末、5Y/10Y=月末。EMA 与 ZigZag 主升浪识别仍在日线算完，再降采样取末日值,精度不损失只是显示更稀
3. **新增 §1.6 王朝接力图**：同一份 RS+Z 强势分换种画法,行=板块、列=月份、色块=金/银/灰。让「连续戴金=王朝期」「金→银→灰=退位换班」在视觉上一目了然

**§1.6 王朝接力规则（与主理人当场敲定）**：

- 指标:RS+Z 合成（跟 §1.5 一致）
- 染色:Top1 = 🥇 金 / Top2-3 = 🥈 银 / 其他 = ⚪ 灰
- 加冕门槛:Top1 必须 RS_252d > 0(年化跑赢 SPY)才戴金,否则降为灰——熊市无王
- 时间轴:5Y = 60 个月格(默认)/ 10Y = 120 个月格(radio 切换)
- Y 轴排序:累计戴金月数降序,金平局按累计戴银

**改动**：

1. `pages/0_宏观雷达.py`:
   - L237 caption 加「采样粒度自动联动」说明
   - L340 ZigZag 算完后插入 resample 逻辑(1Y→W-FRI,5Y/10Y→ME,其他不变)
   - L353 `_TOP_N = 8` → `5`,`_RISING_N = 5` → `2`,注释同步
   - L541 后新增 §1.6 §1.6 板块王朝接力图(约 180 行):radio + heatmap + 王朝统计摘要 + 明细 expander

**后端**：无改动,完全复用 `fetch_macro_radar_timeseries(window="5Y"/"10Y")`

**验收**：

- §1.5 5Y/10Y tab 切到月线,X 轴标签变成月末日期,主升浪着色仍完整
- §1.5 默认彩色 7 条,其他 18 个板块淡灰 + legendonly
- §1.6 渲染 25 行色块矩阵,Y 轴累计金多的板块在最上,最近一列对应当前月
- §1.6 摘要显示「当前在位之王」(Top1+RS>0)和「累计王朝长度 Top3」
- §1.6 5Y/10Y radio 切换正常,色块数量变 60/120

**遗留**：

- 王朝接力图当前固定 RS+Z,未来如果加「纯 RS」「纯 Z」 toggle 是同一份组件改 metric 字段
- 「最长连续戴金」算法是单段计数,跨年的同一王朝若中间断 1 月不会被合并;按需后续加「容忍 1 月断档」开关

---

## 2026-05-27 | Page 0 删除 §1.6 潜力波形，潜力信息压缩进 §1.5 摘要

**动因**：主理人审视后认为 §1.6 板块潜力波形（RS − Z）作为独立图表冗余——直接回答「谁是过高、谁是潜力」用单图 + 文字摘要更高效，不需要两张图。同屏铺两张大图反而让用户在两图之间来回扫视、增加心智负担。

**改动**：

1. `pages/0_宏观雷达.py`:
   - 删 §1.6 整块（图、6 tab、调用 `_render_wave_tab(composite_field="composite_value")` 那段）
   - `_render_wave_tab` 函数内新增 `_pot_df` / `_pot_smooth_df`（加载后端 `composite_value` 时序 + 5 日 EMA 平滑），与现有 `_comp_df / _rs_df / _z_df` 并列
   - 摘要 HTML 在「🚀 排名上升 / 🥀 排名下降」下面加一行：「💎 黄金板块 Top3（强且便宜）」 + 「⚠️ 高估警告 Top3（强但贵）」，按当前 `composite_value` 末日值降序取 head(3) / tail(3)
   - §1.5 标题区注释和 caption 去掉「配套 §1.6」字样，改提「摘要里黄金/高估标签」

2. `_render_wave_tab` 函数签名 4 个 kwargs（`composite_field` / `mode_label` / `formula_op` / `hover_label`）**保留**——目前只剩 §1.5 一种调用，但保留以备将来扩展（如 toggle 切换）

**后端**：`composite_value` 字段保留不动，前端摘要消费

**验收**：

- §1.5 下方摘要从 1 行变 2 行 + 增强「读法」说明
- 黄金板块 Top3 按 `composite_value` 降序、高估警告 Top3 按升序
- §1.6 大标题 / 6 tab / 5Y 第二张图整块不见
- §1.5 标题区不再出现「§1.6」「配套潜力波形」字样

---

## 2026-05-27 | Page 0 §1.6 板块潜力波形 (RS − Z) 与 §1.5 强势波形并联

**动因**：§1.5 强势波形（RS + Z）只回答「主升浪在哪」，靠前的板块往往已经偏贵；主理人需要「便宜的强者」候选池视图（V+M 经典因子）。同屏并列后能做四象限判读：黄金板块 / 热门高估 / 价值陷阱苗头 / 明确避开，避免单一信号追高。

**改动**：

1. `pages/0_宏观雷达.py`:
   - `_render_wave_tab` 函数加 4 个 kwargs：`composite_field` / `mode_label` / `formula_op` / `hover_label`，所有"复合分 / 强度波形 / RS + Z"硬编码字符串模板化
   - §1.5 标题改成「📈 板块强势波形 — RS + Z」，caption 补充「跟随主升浪，接近顶部要警惕（配套 §1.6 潜力波形做交叉验证）」
   - 新增 §1.6「💎 板块潜力波形 — RS − Z」，独立 6 tab，传 `composite_field="composite_value"`
   - 现有 6 个 _render_wave_tab 调用改用 for 循环 + zip

2. `api_client.py`：无改动，新字段随 JSON 透传

**契约**：后端 valuation-radar `compute_radar_timeseries` 同日已新增 `composite_value` 字段（commit `817272a`），见对应仓 DEV_LOG。

**验收**：

- §1.5 / §1.6 上下平铺各 6 tab
- 同一 ticker 在两条波形里排名能不同：XLK 实测 §1.5 末日 +2.48（强势靠前）/ §1.6 -1.07（潜力靠后）= 经典热门高估
- hover / 图标题 / 摘要文案在 §1.6 显示「潜力」而非「强势」
...
1. 主图 Y 轴从离散排名换成连续复合分（强者在图上方、弱者在下方，**Y 轴空间分布自带分层**）。
2. 加 5 日 EMA 平滑（`_comp_smooth_df = _comp_df.ewm(span=5)`），线条像波浪而非日内锯齿。
3. `mode='lines'` 去掉 markers（25 条线上密密麻麻的圆点是噪音）。
4. 加 `y=0` 虚线作"基准 SPY"参考线——线在上方 = 强于大盘，线在下方 = 弱于大盘。
5. legend 按当前复合分降序排序（强者在 legend 顶部），方便对色找线。
6. Hover 同时显示平滑值 + 原始复合分 + RS + Z（4 项）。
7. 排名变化摘要卡片**保留**——"零售 #11→#3" 比纯数值变化更有故事性，与波形图互补。
8. 子模块标题从 "Bump Chart" 改为 "Sector Strength Waveform"。

**保留**：4 tab (1M/3M/6M/1Y)、板块范围跟随 sidebar `selected_groups` 联动、排名 `_rank_df` 仍在前端基于选中组别动态算（仅供摘要卡片消费）。
...
1. `api_client.py` 新增 `fetch_changepoint()`（TTL=3600s + 502/504 自动重试一次）。
2. `pages/0_宏观雷达.py`：
   - 顶部 import 加 `fetch_changepoint`
   - caption 加「变点检测」
   - 侧边栏强制刷新按钮加 `fetch_changepoint.clear()`
   - 在 §2.5（风格传导链）与 §3（市场分化证据链）之间插入新 **§2.6 变点检测**
3. §2.6 内容三块：4 色 banner（无/弱/强/确认 + 触发变量数 + 持续天数进度条）+ 变量触发明细表（按 |z| 降序，CUSUM+/- 染色）+ 近 250 日触发变量数折线图（叠加 K=4 阈值线 + 当前点高亮）+ 折叠白盒解读。

**架构守约**：CUSUM 算法 + 14 变量拼装 + 4 级信号判定全在后端 `macro_engine.compute_changepoint_detection()`，前端只读 dict 渲染（守「业务逻辑不下沉前端」红线）。算法常量（k=0.5 / h=5.0 / K=4 / confirm_days=5）由后端 dict `constants` 字段下发，前端文案模板插值消费——后端改常量前端自动同步。
...
## 2026-05-26 | D 组阶段 3 白盒展示韧性徽章 + 接力来源标签

`pages/3_资产细筛.py` Panel A 守擂明细表新增「韧性」列：`veteran_fuel` 非零时显示 `🛡️ +1.5` 绿色徽章，缺字段或未命中显示 `—`。Panel B 决策推演新增三个 action 类型映射（`HANDOFF_RELAY_V2` 接力转场 v2 / `HANDOFF_SKIPPED_COOLDOWN` 频次保护 / `CASH_GATE_V2` 清仓 v2 / `SKIP_VACUUM_ENTRY` 真空不补 / `SKIP_EPISODE_BLOCK` 拒补给），HANDOFF_* 行额外渲染「接力来源」标签（绿色 `narrative_handoffs` / 灰色 `rotation_summary` / 橙色 `ticker cooldown`），后端关闭 handoff_v2 时降级显示。
...
## 2026-05-20 | Universe 扩容 W5A 收尾验收（结构验收 + day-0 基线）

- 完成 W5A 执行：W5 备份、90 天 V3 force 回填、扩容前后对比、C 桥 day-0 baseline、健康巡检、只读 D 守擂回放、综合报告。
- V3 回填结果：`latest_common_date=2026-05-19`，38 个可用日期全部 computed，0 failed。
- 扩容基线使用 W2 前备份：universe active `116 → 149`，affinity active `865 → 998`，T1 dictionary `166 → 188`；W4 备份只用于 ETF 隔离语境，不作为扩容前。
- 四个最薄 L2 全部过 `>=5`：Semi `2→6` / Space `2→5` / Cyber `2→5` / Crypto `3→5`（breadth eligible 口径，ETF/proxy 不计入 breadth）。
- C 桥只做 day-0，不做 pass/fail：33 新 ticker 全部 `bridge_confidence >= medium`，`C>0 = 9/33`，`low/none = 0`；T+7 复验 `2026-05-27`，T+14 复验 `2026-06-03`。
- 健康巡检：`stale_factor=0`，`narrative_orphan=0`，新增冷启动 ticker 未误判；唯一结构提醒是 `US_Industrial_Reshoring` breadth eligible 只有 3。
- 退场机制选 B：不新增 `universe.status` schema，不自动 archive，只输出 watch / archive / proxy_only 建议清单。综合报告见 `data/universe_expansion_w5_report.md`。

---

## 2026-05-20 | Universe 扩容 W1 设计表产出（仅纸面，不动 DB）

- 产出 `valuation-radar/data/universe_expansion_plan.md`，覆盖 20 个 active 产业 L2 的扩容设计。
- 本轮提议新增 33 个 ticker（29 stock + 4 ETF），重点补 8 个最薄 L2：Space（2→6）/ Crypto（3→6）/ Semi_Memory（3→6）/ Cybersecurity（3→7）/ Agri / Autonomous / Biopharma / Housing。
- 5 条机器口径自检全通过（`scripts/validate_expansion_plan.py`），133 个 required_affinity_terms 全部命中现行 active 词典，**W3 无须为本表新增词条**。
- 5 项决策主理人同日裁决完成：MSTR=Crypto_Assets / MARA=A 桥单核不动 S 桥 map / IBIT=proxy_only 不进 V3 / BTC-USD/ETH-USD asset_type=crypto / 18 个 ETF 统一暂不进 V3 主排序，W4 后用 proxy_weight=0.2-0.3 低权重进入。
- W2 须同时补 5 条 S 桥 map：Semiconductor Equipment & Materials / Agricultural Inputs / Auto Parts / Medical Care Facilities / Data Storage。
- 本窗口不 commit（设计表是工作文档，待主理人决策后 W2 一起提交）。
...
### chaos 月度占比强制 BIL 保护 Phase 2 — 中断待续

**计划**：`chaos_月度占比强制_bil_保护_b40196b6.plan.md` Phase 2（前端），在后端 Phase 1 已推 Render 后执行。

**中断原因**：
- 执行前按规程 curl Render 验证：`POST /api/v1/macro/compute {"z_window":750}`
- Render 返回 `success: false, error: "compute_macro_regime returned empty"`
- 接口报错，无 `horsemen_monthly_probs` 字段，前提条件未满足，Phase 2 未动手

**待排查（明天）**：
1. 确认后端 Phase 1 commit 是否已推到 main 并在 Render 部署成功
2. Render deploy logs 有无构建/运行报错
3. 持久盘 `/opt/render/project/src/data/narrative.db` 是否在位（`compute_macro_regime` 依赖 narrative.db）
4. 排查完毕、接口正常返回含 `chaos_share` 的 `horsemen_monthly_probs` 后，再执行 Phase 2

**Phase 2 改动点（待执行）**：
- `pages/1_宏观定调.py`：月度表增「chaos 占比%」列，> 40% 深灰粗体
- `pages/6_仓位配置.py`：rebal 表消费 `regime_dormant_reason`，chaos-dormant 月显示 `dormant (chaos)` 并染色
...
## 2026-04-25 | 回填切片 `_CHUNK` 从 6 放回 12（Render 升 Standard 2 GB 后的性能调优）

**动因**：Render 后端 2026-04-25 从 Starter (512 MB) 升级到 Standard (2 GB)，内存翻 4 倍。`api_client.arena_backfill_score._CHUNK` 当初 12→6 是给 Starter 内存留缓冲，现在余量充足，回填 72 个月（60 + 12 热身）的批次数从 12 批减回 6 批，速度直接翻倍。

**改动**：`api_client.py` `_CHUNK = 6` → `_CHUNK = 12`，注释同步更新。

**决策依据**：
- 12 是历史原始值，已知能跑（Starter 是 OOM 才降的，不是 12 本身有 bug）
- 内存 4 倍 → `_CHUNK` 只翻 1 倍，留足余量
- 单批月数过大会撞 Render 网关代理超时（每批后端逐月打分，越大越慢），先一次只翻一档便于回滚验证
- 12 跑稳后未来再考虑 18 / 24

**风险与回滚**：若 Standard 仍 OOM 或代理超时，把 `_CHUNK` 改回 6 即可，每批落盘逻辑（partial 返回 / `push_arena_history_batch` 即时写）全部保留，不会"中途挂全白跑"。

---
## 2026-06-10 | 王朝接力图准确性修复（前端批次一、二）

### 改动

- 王朝时间跨度移除 1Y/2Y，仅保留 3Y/5Y/10Y，默认仍为 5Y。
- 根据后端 `asof` 与月末标签判断未结束月。热力图保留当月格并标注“进行中”，
  但累计戴金、累计戴银、最长连续戴金及排序均只使用已确认月份。
- 摘要在未结束月显示“当月领先(进行中·未定格)”，不再称为“当前在位之王”。
- 龙头 tab 在王朝分段前剔除进行中月；若当月金牌延续最后一个已确认金块，
  正式段仍截止最后一个已确认月，并在王朝期旁标注“当月进行中”。
- 龙头结果消费后端 `stale_endpoint`：个股端点落后 ETF 超过 5 个交易日时显示 ⚠️
  并在状态列列出 ticker。

### 核验

- 2026-04/05/06 合成场景：SMH 正式持续月数为 2，API 截止 2026-05 月末，
  2026-06 仅作为进行中延续展示。
- `python3 -m py_compile pages/0_宏观雷达.py` 通过。

### 已知保留项

- 候选池仍使用当前成分快照回看历史，存在幸存者偏差。
- `shift(253)` 与节假日填充口径、66 vs 25 横截面标准化母体本批次不改，
  需在会改写历史金牌的后续批次单独全量核验。

---
## 2026-07-12 | page8 股池换成 fcf+roic 规则池，排名轴换 FCF margin

后端已切 page8 到「判据1+2 + ROIC≥10% + FCF margin 前 40」年度 PIT 规则池
（删掉原「回购+分红≤FCF」反向判据），本批同步前端。

### 改动

- `api_client.py` 新增 `fetch_buyback_stable_relay_timeseries`，
  URL 换 `/api/v1/macro/buyback_stable_relay/timeseries`。
- `pages/8_回购稳定.py`：标题/caption 改为规则池说明；排名字段
  `shareholder_yield`→`fcf_margin`（变量同步 `shy_*`→`fcfm_*`）；
  删 `_TECH_TICKERS` 硬编码列表，非科技子集改读后端 `is_tech`；
  照抄 page 7 的退市票 Sharadar 补全块（`fetch_gbdt_oos_prices` + `holdings_viz`）。
  留任 MA4 卖出 / 买回 MA15 门、金银牌规则不动。

### 核验

- `py_compile` 通过，无 lint 新增错误。
- 线上端点 `buyback_stable_relay/timeseries?window=3Y`：2016-2026 每年池 ≤40 只，
  2026 非科技 20 只（含 V/MA/MSCI/MO/UNP，不含 DFS/CACC/SYF/OMF/COF），
  `V.fcf_margin` 末值 49.24、`is_tech=false`。
- page 7 / page 18 端点回归确认无报错。
